...
namespace Itenso.Windows.Controls.ListViewLayout
{
	...
	public class ListViewLayoutManager
	{
		...
		public static readonly DependencyProperty AddExtraSpace = DependencyProperty.RegisterAttached(
				"AddSpace",
				typeof(bool),
				typeof(bool));
		...
		// ----------------------------------------------------------------------
		public static void SetAddSpace(DependencyObject dependencyObject, bool enabled)
		{
			dependencyObject.SetValue(AddExtraSpace, enabled);
		} // SetAddSpace
		...
		protected virtual void ResizeColumns()
		{
			...
			if ( resizeableRegionCount <= 0 )
			{
				...
				if ( fillColumn != null )
				{
					...
					double fillWidth = actualWidth - otherColumnsWithoutFillWidth - addSpace;
					if ( fillWidth > 0 )
					{
						...
						{
							this.scrollViewer.HorizontalScrollBarVisibility = ScrollBarVisibility.Hidden;
							fillColumn.Width = fillWidth - addSpace - 2;
						}
					}
				}
				return;
